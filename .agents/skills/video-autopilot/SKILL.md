...
2. 確認走路徑 A-E（本技能），還是路徑 F（見下方）。
3. 再決定要走 `CapCut`、`JSON` 直接編輯，還是 `ffmpeg` 路徑。
4. 如果任務還沒講清楚，先補最少必要資訊，不要急著動剪。
5. 完成輸出後，記得做 QA 和結果紀錄。
...
## 路徑 F：改用 video-use-editor

如果素材符合以下條件，應切換到 `video-use-editor`，不要用本技能：

- 素材主體是**口播、講話、訪談或教學說明**
- 需要**自動逐字稿 + 精準剪掉 uh/um/贅詞/死空間**
- 有**多 take 素材**需要 AI 智慧選取
- 需要**word-level 精確字幕對齊**（非 CapCut 視覺字幕）

切換方式：直接告知使用者改用 `video-use-editor` 技能。
...
- 路徑 E：純 `ffmpeg`。
- 路徑 F：口播精剪 → 改用 `video-use-editor`（非本技能範圍）。
