...
- **看板更新**：執行 `generate_dashboard_data.py`，重新爬取 482 支個股收盤價並產生 `dashboard_data.js`。
- **今日重點**：00405A 南電（8046）暴力加碼 +2.34%（2.83% → 5.17%）；環球晶（6488）多 ETF 同步加碼；旺矽（6223）、國巨（2327）多 ETF 同步減碼；00402A 新增 SK hynix、刪除 ON Semi。

## [2026-07-13] etf-update | 主動型 ETF 第 37 次日更
- **資料更新**：執行 `collect_active_etf_holdings.py` 抓取並追加 `20260713` 分頁至 `主動型ETF持股明細.xlsx`（累計 37 個歷史日期分頁）。共 29 支 ETF、1,643 筆持股明細。
- **每日個股合計**：執行 `add_daily_stock_total.py`，更新「每日個股合計」至 26,790 筆。自動生成加碼/減碼 Top 10 圖表並嵌入 `主動型ETF持股變動.md`。
- **看板更新**：執行 `generate_dashboard_data.py`，重新爬取個股收盤價並產生 `dashboard_data.js`（481 支股票，37 個日期分頁）。
- **今日重點**：00405A 南電（8046）大幅加碼（+2.34%）；00985A 台積電大幅減碼（-2.65%）；00988A 大換股（新增 Fortinet、ServiceNow）；00987A 新增南電。

## [2026-07-11] ingest | AI生圖提示詞與視覺版面指南
- **資料收錄**：Ingest `raw/圖片提示詞/` 資料夾下的原始資料。
- **建立卡片**：於 [AI繪圖與提示詞庫解析.md](file:///I:/Mark/my-kb/cards/AI繪圖與提示詞庫解析.md) 建立暫時筆記，分析高訂時尚攝影核心描述，並提煉 Excel 檔案內 57 個 Sheet 的精華提示詞、新手六大核心及進階視覺指令。
- **建立 Wiki 頁面**：新增 [AI生圖提示詞與視覺版面指南.md](file:///I:/Mark/my-kb/wiki/AI工具/AI生圖提示詞與視覺版面指南.md) 永久筆記，提供系統化的 AI 繪圖 Prompt 架構與 15 大商業/社群特色視覺（如 3D 樹脂、韓風手帳、側臉剪影等）實戰指引。
- **索引更新**：於 [index.md](file:///I:/Mark/my-kb/index.md) 註冊新 Wiki 頁面。
...
## [2026-09-18] active-etf | 主動型 ETF 第 83 次日更與週彙總、個股合計同步
- **數據採集與比對**：成功運行 `collect_active_etf_holdings.py` 爬取今日（2026/09/18）30 檔主動型 ETF，共 1,736 筆持股，寫入 `wiki/金融投資/主動型ETF持股明細.xlsx` 的 `20260918` 工作表（比較區間：`20260917` → `20260918`）。
- **每週加減碼明細計算**：執行 `add_weekly_summary.py`，順利計算並更新 Excel 中的「Weekly Additions」與「Weekly Reductions」工作表，完成本週（截至 09/18）持股變動分析。
- **每日個股合計同步**：執行 `add_daily_stock_total.py`，順利在 Excel 中更新「每日個股合計」工作表，累計共 83 個交易日的資料（62,605 列）。
- **Dashboard 數據重新產生**：執行 `generate_dashboard_data.py`，更新 [dashboard_data.js](file:///i:/Mark/my-kb/wiki/金融投資/dashboard_data.js) 供前端網頁載入（包含 563 檔個股收盤價歷史數據）。
- **報告與圖表生成**：同步重新生成 [主動型ETF持股彙總.md](file:///i:/Mark/my-kb/wiki/金融投資/主動型ETF持股彙總.md) 與 [主動型ETF持股變動.md](file:///i:/Mark/my-kb/wiki/金融投資/主動型ETF持股變動.md)，並生成 `20260918_additions.png` 和 `20260918_reductions.png`，更新固定檔名視覺化圖表。
